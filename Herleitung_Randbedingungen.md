...
> die gerenderte Ansicht mit Rechtsklick → _Open Preview_ oder `Strg`+`Shift`+`V`.
...
> **Stufe 1 (`raumtemperatur_a.py`).** Dort sind _alle_ nicht durch Heizung/Tür
...
Ausgeschrieben in Komponenten (das ist der erste Schritt der
Geisterpunkt-Herleitung in §4.2/4.3):

$$
u_{\mathbf n} = \nabla u \cdot \mathbf n
= \frac{\partial u}{\partial x}\,n_x + \frac{\partial u}{\partial y}\,n_y
\stackrel{!}{=} 0.
$$

An einer achsenparallelen Wand hat $\mathbf n$ nur eine von Null verschiedene
Komponente ($n_x=\pm1,\,n_y=0$ oder umgekehrt). Damit fällt einer der beiden
Terme weg, und die Bedingung reduziert sich auf eine einzige partielle
Ableitung — das ergibt die vier Fälle der folgenden Tabelle.
...
**Schritt 4 — explizit nach $u_{0,j}$ auflösen** (genau die Formel, die
unten im Foto rot dasteht, nur mit $u$ statt $f$ und für den linken statt
rechten Rand — vgl. §4.3 Schritt 4):

$$
\boxed{\,u_{0,j} \approx \dfrac{u_{0,j+1} + u_{0,j-1} + 2\,u_{1,j}}{4}\,}
$$
...
Exemplarisch noch einmal **ausführlich** für den **rechten Rand** $x=L_x$
(Spaltenindex $i=N_x{-}1$), mit Skizze der Nachbarpunkte — Spiegelbild von
§4.2, nur mit vertauschten Rollen von innerem Nachbar und Geisterpunkt:
...
```
        ...
        |                      (i, j+1) •
        |                          :
        |    (i-1,j) •    (i,j) •  :  • (i+1,j)   ← Geisterpunkt (liegt außen,
...
**Schritt 1 — Neumann-Bedingung an dieser Wand:** Aus §4.1 mit
$\mathbf n=(1,0)$ folgt $u_{\mathbf n}=u_x$, zentral diskretisiert am
Randpunkt $(i,j)$:

$$
u_x(L_x,y_j) \approx \frac{u_{i+1,j}-u_{i-1,j}}{2h} \stackrel{!}{=} 0.
$$

**Schritt 2 — auflösen:** $u_{i+1,j} = u_{i-1,j}$ (Spiegelung am Rand, der
Geisterpunkt übernimmt den Wert des inneren Nachbarn — hier also
$u_{i-1,j}$, nicht wie in §4.2 den von $u_{i+1,j}$).

**Schritt 3 — in den Stern einsetzen:**

$$
-4\,u_{i,j} + \underbrace{u_{i+1,j}}_{=\,u_{i-1,j}} + u_{i-1,j} + u_{i,j+1} + u_{i,j-1} = 0
\;\Longrightarrow\;
\boxed{\,-4\,u_{i,j} + 2\,u_{i-1,j} + u_{i,j+1} + u_{i,j-1} = 0\,}
$$

**Schritt 4 — explizit nach $u_{i,j}$ auflösen** (die Matrixzeile umgestellt;
nützlich als Update-Formel für iterative Löser wie Jacobi/Gauss-Seidel,
auch wenn der Code hier stattdessen die Matrix $A$ direkt aufbaut):

$$
\boxed{\,u_{i,j} \approx \dfrac{u_{i,j+1} + u_{i,j-1} + 2\,u_{i-1,j}}{4}\,}
$$

Unten ($j=0$) und oben ($j=N_y{-}1$) verlaufen die Wände in $y$-Richtung
und ergeben sich komplett analog (nur $x \leftrightarrow y$ vertauscht):
...
`is_door` werden _vor_ dem `on_boundary`-Block geprüft. Dirichlet hat also
**Vorrang** vor Neumann:
...
### 8.3 Warum das _harmonische_ Mittel an den Kanten?
